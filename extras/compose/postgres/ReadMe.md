...
Original-Verzeichnis `init\chora\sql` bzw. `init\chora\tls` in das Named Volume
...
das Named Volume `pg_init` kopiert. Danach wird das Verzeichnis `/tablespace/chora`
...
DB `chora` mit dem DB-User `chora`anzulegen. Mit dem 2. Skript wird das
Schema `chora` mit dem DB-User `chora` als _Owner_ angelegt:
...
Mit der Erweiterung _PostgreSQL_ für VS Code kann man die Datenbank `chora` und
...
Im Untermenü _Databases_ von der Verbindung sieht man dann z.B. die Datenbank `chora`
mit dem gleichnamigen Schema `chora` und die Datenbank `postgres`.
Ebenso kann man man unter _Roles_ den DB-User `chora` und den Superuser `postgres`
...
- _Patienten_ als Plural der DB-Tabelle `chora`.
